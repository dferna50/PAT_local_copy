...
async function pushToCompilationCache(transport2, cache) {
  if (workerShouldFallbackCompilationCache) {
    transport2.send("pushToCompilationCache", { cache }).catch((e) => (0, import_util.debugTest)("Failed to push compilation cache", e));
    return;
  }
  const { timedOut } = await (0, import_utils.raceAgainstDeadline)(() => transport2.send("pushToCompilationCache", { cache }), (0, import_utils.monotonicTime)() + 1e3);
  if (timedOut) {
    (0, import_util.debugTest)("Falling back to unawaited compilation cache");
