...
class Page extends import_instrumentation.SdkObject {
  ...
  async resetForReuse(metadata) {
    ...
    await this.setServerRequestInterceptor(void 0);
...
`).join("");
  ...
  async setServerRequestInterceptor(handler) {
...
class InitScript {
  // Can be arbitrarily used by a browser-specific implementation.
...
async function snapshotFrameForAI(frame, frameOrdinal, frameIds) {
  ...
  for (const line of lines) {
    ...
    try {
      const childSnapshot = await snapshotFrameForAI(child.frame, frameOrdinal2, frameIds);
